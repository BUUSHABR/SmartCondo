...
public class AutoStartHelper {
    ...
    public void getAutoStartPermission(Context context) {
        Log.d("Editable", "append: ");


        String build_info = Build.BRAND.toLowerCase();
        Log.d("Editable", "append: " + build_info);

        Log.d("build info","" + build_info);
        switch (build_info) {
//            case BRAND_ASUS:
...
//                break;
    ...
    private void autoStartOthers(final Context context) {
        try{
    ...
    private void autoStartAsus(final Context context) {
        if (isPackageExists(context, PACKAGE_ASUS_MAIN)) {
            ...
            showAlert(context, (dialog, which) -> {
                try {
                SharedPrefsUtils.setBooleanPreference(context, SharedPrefsUtils.PREF_KEY_APP_AUTO_START, true);
